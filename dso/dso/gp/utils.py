...
class Individual(gp.PrimitiveTree):
    ...
    def __init__(
        self, actions, pset, max_mutations, ind_representation, master_sequence
    ):
...
def cxModifiedPMX(ind1, ind2, **kwargs):
    ...
    idx_notsel = list(range(cxpoint1)) + list(range(cxpoint2, size))
    ...
    for i in range(size):
        if random.random() < indpb:
            ...
            individual.work_repr[i], individual.work_repr[swap_indx] = (
                individual.work_repr[swap_indx],
                individual.work_repr[i],
            )
...
def DEAP_to_tokens(individual):
    ...
    return np.array([i.name for i in individual], dtype=np.int32)
