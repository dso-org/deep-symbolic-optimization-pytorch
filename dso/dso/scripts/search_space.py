...
def count_violations(actions, obs, model):
    ...
    lengths = np.array(
        [min(len(p.traversal), model.policy.max_length) for p in programs],
        dtype=np.int32,
    )
    ...
    T = actions.shape[1]
    return sum(
        (priors[i, np.arange(T), actions[i]][: lengths[i]] == -np.inf).any()
...
@click.option(
    "--mode", type=click.Choice(["marginal", "single", "all", None]), default=None
)
def main(config1, config2, n, mode):
    ...
    if mode is None:
        ...
        assert (
            tokens1 == tokens2
        ), "Tokens must be the same between config1 and config2."
        count = count_violations(actions, obs, model2)
