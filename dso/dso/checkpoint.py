...
class Checkpoint:
    ...
    def save(self, save_path=None):
        ...
        if save_path is None:
            ...
            save_path = os.path.join(self.checkpoint_dir, f"checkpoint_{timestamp}")
        ...
        # Save the policy and optimizer
        policy_save_path = os.path.join(save_path, "policy.pt")
        torch.save(self.model.policy.state_dict(), policy_save_path)
        optimizer_save_path = os.path.join(save_path, "policy_optimizer.pt")
        torch.save(
            self.model.policy_optimizer.optimizer.state_dict(), optimizer_save_path
        )
    ...
    def load(self, load_path):
        ...
        if self.model.device is None:
            self.model.setup()
        policy_load_path = os.path.join(load_path, "policy.pt")
        optimizer_load_path = os.path.join(load_path, "policy_optimizer.pt")
        self.model.policy.load_state_dict(
            torch.load(policy_load_path, map_location=self.model.device)
        )
        self.model.policy_optimizer.optimizer.load_state_dict(
            torch.load(optimizer_load_path, map_location=self.model.device)
        )
