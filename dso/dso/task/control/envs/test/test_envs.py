...
env_ids = [
    ...
    "CustomCartPoleContinuousBulletEnv-v0",
]
dts = [0.01, 0.02]
...
for env_id in env_ids:
    ...
    for dt in dts[1:]:
        assert np.array_equal(
            s0, data[env_id][dt]["s0"]
        ), "Starting states for {} are not identical.".format(env_id)

    # Assert next states are not identical
    s1 = data[env_id][dts[0]]["s1"]
    for dt in dts[1:]:
        assert not np.array_equal(
            s1, data[env_id][dt]["s1"]
        ), "Next states for {} are identical.".format(env_id)
